...
HWND BasicWindowMaker::create_window(wchar_t* p_title, wchar_t* p_windowClass, int p_x, int p_y, int p_width,
	int p_height, WNDPROC p_wndProc, HINSTANCE p_hInst, int p_nCmdShow, int exWindowStyles, int windowStyles,
	int classStyles, int menuRc, int iconRc, int iconSmallRc, HWND parent)
{
	MyRegisterClass(p_hInst, p_wndProc, p_windowClass, classStyles, iconRc, iconSmallRc, menuRc);
	return InitInstance(p_title, p_windowClass, p_x, p_y, p_width, p_height, p_hInst, exWindowStyles,
		windowStyles, p_nCmdShow, parent);
}
